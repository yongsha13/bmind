    ...
    $('#mn')
        ...
        .on('click','.home-list li .min-player',function(){
            ...
            bmApi.api('player',data_1,function(res){
                var data_2 = $.extend({},data_1,{musicId:data_1.playId||data_1.id,versionCode:1,userSource:2});
        ...
        .on('click','.js-more-index',function(){
            ...
            var data = $(this).data();
            if(window['isIndexLoading']) return false;
            window['isIndexLoading'] = true;
            $.get('/BmindAPPSet/app/home/100/list.do?rows=10&page='+data['page'],window.params['args']||{},function(res){
                data['page'] = parseInt(data['page']) +1;
                data['list'] = res.data;
                console.log(res.data.length);
                $(_this).closest('li').replaceWith(TPL.render('bmindIndexList',data));
                window['isIndexLoading'] = false;
            })
        })
        /*更多心理阅读*/
        .on('click','.js-more-read',function(){
            var _this = this;
            var data = $(this).data();
            $.get('/BmindRepository/app/article/100/articleList.do?rows=10&page='+data['page'],window.params['args']||{},function(res){
                data['page'] = parseInt(data['page']) +1;
                data['list'] = res.data;
